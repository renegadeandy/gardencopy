                    ...
                    for (int x = -radius; x < radius; x++) {
                        for (int z = -radius; z < radius; z++) {
                            ...
                            if (Mathf.Abs( newPos.sqrMagnitude) < radius) {
                                ...
                                try {
                                    Vector4 save = (mg.storedData[ConvertVec3ToInt(newPlaceAt)]);
                                    save.w += remove*0.1f;
                                   // save.w *= remove;
                                    save.w = Vmath.MinMaxValue (save.w, -1, 1);
                                    mg.storedData[ConvertVec3ToInt(newPlaceAt)] = save; // new Vector4(newPlaceAt.x, newPlaceAt.y, newPlaceAt.z, Random.Range(-15,15));
